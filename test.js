...
UICatalogService.prototype.mergeUI5LibraryPreloadFiles = function(libType, libVersion, libraries) {
    var self = this;
    ...
    return Promise.all([sapUICorePromise, mergedLibrariesPromise]).then(function(fileContentArray){
        ...
        return self.storeNewFileFromString('sap-ui-core-preloaded.js', fileContent, libType, libVersion, false);
    }).catch(function(error){
        deferred.reject('Unable to generate sap-ui-core with preloaded library files : ' + error);
    });
};

UICatalogService.prototype.mergeUI5LibraryCSSFiles = function(libType, libVersion, libraries, themeName) {
    var self = this;
    ...
    return Promise.all(cssFileContentPromises).then(function(libraryStringArray){
        ...
        return self.storeNewFileFromString('sap-ui-merged-libraries.css', mergedCssFileContent, libType, libVersion, false);
    });
};

UICatalogService.prototype.generateAndStoreMergedUI5Files = function(libType, libVersion) {
    var tasks = [];
    var libraries = ['sap/ui/core', 'sap/ui/layout', 'sap/ui/commons', 'sap/m', 'sap/ui/unified'];
    tasks.push(this.mergeUI5LibraryCSSFiles(libType, libVersion, libraries, 'sap_bluecrystal'));
    tasks.push(this.mergeUI5LibraryPreloadFiles(libType, libVersion, libraries));
    return Promise.all(tasks);
};

UICatalogService.prototype.initializeLibrary = function() {

    var initLibaryDeferred = Promise.defer();
    ...
    fs.readFile(self.readAndGenerateZip(), function(err, data) {
            ...
            filePromises = _.map(openui5zip.files, function(fileObject) {
                var deferred = Promise.defer();
                self.storeNewFile(fileObject, libraryType, libraryVersion, false).then(
                    function(result) {
                        deferred.resolve(result);
                    },
                    function(err) {
            ...
            Promise.all(filePromises)
                .then(function(files) {
                    self.generateAndStoreMergedUI5Files(libraryType, libraryVersion).then(function(){
                        initLibaryDeferred.resolve();
                    }).catch(function(error){
                        initLibaryDeferred.reject('Unable to generate merged UI5 files: ' + error);
                    });
                });
        });
    return initLibaryDeferred;
};
