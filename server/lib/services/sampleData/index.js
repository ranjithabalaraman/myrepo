...
SampleDataService.prototype.initialize = function(done) {
  this.mongooseModel = dataManager.initialize(done);
};
 ...
 */
SampleDataService.prototype.updateSDfromProjId = function(dataModelJson, sampleData, waitTiltAllEventsProcessed, user) {
  if (!dataModelJson) {
  ...
  return dataManager.updateSD(dataModelJson, SDDoc, true, waitTiltAllEventsProcessed, user).then(function(result) {
    if (!result) {
      return null;
    }
...
SampleDataService.prototype.updateEntitiesfromProjId = function(dataModelJson, sampleDataUpdated, waitTiltAllEventsProcessed, user) {
  ...
  return dataManager.getSDfromProjId(projectId, null, waitTiltAllEventsProcessed)
    .then(function(sampleDataModel) {
      if (sampleDataModel) {
        ...
        var SDDoc = new SDModel(sampleData);
        return dataManager.updateSD(dataModelJson, SDDoc, true, waitTiltAllEventsProcessed, user);
      }
    }).then(function(result) {
    if (!result) {
      return null;
