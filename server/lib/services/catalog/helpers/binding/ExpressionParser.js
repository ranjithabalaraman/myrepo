...
function error(sMessage, sInput, iAt) {
    ...
    console.error(sMessage, sInput, 'sap.ui.base.ExpressionParser');
    throw oError;
}
...
module.exports = {
    ...
    parse: function (fnResolveBinding, sInput, iStart, mGlobals) {
        ...
        var cleanedToken = [];
        _.each(oTokens.tokens, function(token) {
            if(token.id != '}') {
                var newToken = {id : token.id};
                if(token.value != null) {
                    if(token.id == 'BINDING') {
                        newToken.value = oTokens.parts[token.value];
                    }
                    else {
                ...
                cleanedToken.push(newToken);
            }
        });
        return {
            result: {
                expression: cleanedToken,
                parts: oTokens.parts
