...
var ExpressionParser = require('./ExpressionParser.js');
...
BindingParser.simpleParser = function (sString, oContext) {
    ...
    if (_.startsWith(sString, '{') && _.endsWith(sString, '}')) {
...
BindingParser.complexParser = function (sString, oContext, bUnescape) {
    var parseObject = ParseJS().parseJS,
        oBindingInfo = {parts: []},
        aFragments = [],
        bUnescaped,
        p = 0,
    ...
    function resolveRef(o, sProp) {
        return; // No need to do anything in a non UI5 environement
        //if (typeof o[sProp] === "string") {
        //    var sName = o[sProp];
        //    if (_.startsWith(o[sProp], ".")) {
    ...
    function resolveEvents(o, sProp) {
        if (!(_.isPlainObject(o[sProp]))) {
            return;
        }
        _.each(o[sProp], function (oObject, sName) {
            resolveRef(o[sProp], sName);
        });
    }

    function resolveBoolean(o, sProp) {
        if (typeof o[sProp] === 'string') {
            o[sProp] = (o[sProp] === 'true');
        }
    }

    function resolveObject(o, sProp, sParentProp) {

        if (!(typeof o[sProp] === 'object' || _.isArray(o[sProp]))) {
            return;
        }
        if (_.isArray(o[sProp])) {
            _.each(o[sProp], function (oObject, iIndex) {
    ...
    function resolveEmbeddedBinding(sInput, iStart) {
        ...
        if (rObject.test(sInput.slice(iStart))) {
            ...
            resolveBoolean(oParseResult.result, 'forwarded');
