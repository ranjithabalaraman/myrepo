...
UICatalogService.prototype.processSmartTemplates = function(files, libType, libVersion, isPrivate) {
    ...
    if (smartTemplateLibFiles.length > 0) {
        var catalog = smartTemplateHelper.extractCatalog(smartTemplateLibFiles);
        ...
        this.addActionsToCatalog(catalog);
        ...
        if (_.keys(catalog.controls).length > 0) {
            self.createCatalog(catalog).then(function(stRootCatalog) {
                ...
                delete stCustomCatalog.actions;
...
UICatalogService.prototype.readAndGenerateZip = function() {
    var zipfile = 'node_modules/norman-openui5/openui5.zip';
    var openui5dir = 'node_modules/norman-openui5';
    try {
        fs.unlinkSync(zipfile);
    } catch (err) {
        logger.info('error in removing the openui5 library or file already deleted ');
    }
    readDirectory(openui5dir, []);
    var genZip = zip.generate({
        type: 'nodebuffer'
    });
    fs.writeFileSync(zipfile, genZip);
    return zipfile;
};

UICatalogService.prototype.storeOpenUI5Canvas = function(callback) {
...
UICatalogService.prototype.storeNewFile = function(zipEntry, libType, libVersion, isPrivate) {
    ...
    readStream.on('error', function(err) {
        logger.err(' in the error of readstream ');
        deferred.reject(err);
    });
    return deferred.promise;
};
